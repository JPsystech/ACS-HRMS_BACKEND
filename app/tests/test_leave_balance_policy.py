...
def policy_2026(db: Session):
    """Ensure ACS policy exists for 2026 (PL=6, CL=5, SL=6, RH=1, FL=1)."""
...
def test_new_employee_has_leave_balances_after_create(db: Session, new_employee_full_year, policy_2026):
    ...
    assert types >= {LeaveType.PL, LeaveType.CL, LeaveType.SL, LeaveType.RH, LeaveType.FL}
    by_type = {b.leave_type: b for b in balances}
    assert float(by_type[LeaveType.PL].remaining) <= 6
    ...
    assert float(by_type[LeaveType.FL].remaining) <= 1


def test_balance_me_returns_entitled_used_available(client, db: Session, dept, policy_2026, new_employee_full_year):
    """GET /leaves/balance/me returns year and {PL, CL, SL, RH, FL} with entitled, used, available."""
...
def test_public_summary_returns_acs_entitlements(client, db: Session, policy_2026):
    """GET /policy/{year}/public-summary returns PL=6, CL=5, SL=6, RH=1, FL=1, public_holidays=14."""
