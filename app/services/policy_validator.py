...
def get_or_create_policy_settings(db: Session, year: int) -> PolicySetting:
    ...
    2026 FINAL policy defaults:
    - Annual entitlements: PL=6, CL=5, SL=6, RH=1, FL=1
    - Monthly credit: +0.5 PL, +0.5 SL every month; +0.5 CL Jan–Oct; +0.5 FL Nov–Dec
    - PL/FL eligibility: 6 months
...
def validate_pl_eligibility(
    ...
    """
    if leave_type not in (LeaveType.PL, LeaveType.FL):
        return
    
    if override_policy:
        return  # Override bypasses PL eligibility check
    
    # Calculate eligibility date (join_date + 6 months)
    ...
    # Check if from_date is before eligibility date
    if from_date < pl_eligible_date:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"PL/FL is allowed only after {settings.pl_eligibility_months} months from join date. "
