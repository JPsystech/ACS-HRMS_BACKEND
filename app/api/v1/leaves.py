...
async def balance_me(
    ...
    for b in balances:
        ...
        notes = None
        if b.leave_type.value in ("PL", "FL") and not eligible:
            notes = "PL/FL usable only after 6 months from join date"
        allocated = float(info.get("total_entitlement", 0)) + float(b.opening) + float(b.carry_forward)
        locked = float(b.accrued) if (b.leave_type.value in ("PL", "FL") and not eligible) else 0.0
        items.append(
            BalanceItemOut(
