...
async def submit_correction(
    ...
    if rt == CorrectionRequestType.FORGOT_PUNCH_IN and not payload.requested_punch_in:
        raise HTTPException(
            ...
            detail="Punch-in time is required when selecting 'Forgot Punch In'"
        )
    if rt == CorrectionRequestType.FORGOT_PUNCH_OUT and not payload.requested_punch_out:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail="Punch-out time is required when selecting 'Forgot Punch Out'"
        )
    if rt == CorrectionRequestType.CORRECTION and not (payload.requested_punch_in or payload.requested_punch_out):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail="At least one punch time (in or out) must be provided for a correction request"
        )
