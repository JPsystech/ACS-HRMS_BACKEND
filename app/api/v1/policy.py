...
async def get_policy_public_summary(
    ...
    If policy does not exist, it is created with ACS 2026 defaults (PL=6, CL=5, SL=6, RH=1, FL=1, public_holidays=14).
    """
...
async def get_policy_endpoint(
    ...
    """Get policy settings for a year (Admin-only). Auto-create defaults if missing."""
    # Use get_or_create to avoid 404 and ensure the Admin UI works on a fresh database
    policy = get_or_create_policy_settings(db, year)
    return policy
...
async def update_policy_endpoint(
    ...
    return update_policy_settings(
        ...
        annual_fl=policy_data.annual_fl,
